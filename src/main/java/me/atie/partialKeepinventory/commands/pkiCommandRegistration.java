...
public class pkiCommandRegistration {
    ...
    private static void modeMessage(CommandContext<ServerCommandSource> ctx){
        ctx.getSource().sendFeedback( Text.literal(
                        "Keepinventory mode is set to " + CONFIG_COMPONENT.partialKeepinvMode().toString()
                ), true);
        ...
        dispatcher.register(literal("pki").requires(source -> source.hasPermissionLevel(4))
                ...
                .then(literal("info").executes(
                        ctx -> {
                            ...
                            String message = "Mod " + (CONFIG_COMPONENT.isEnabled() ? "enabled" : "disabled") + "\n";

                            message +=  "---Inventory---\n" + ">Keepinventory mode: " + CONFIG_COMPONENT.partialKeepinvMode().toString() + "\n";

                            // only show relevant information
                            message += switch(CONFIG_COMPONENT.partialKeepinvMode()){
                                case STATIC ->
                ...
                .then(literal("inv")
                    .then(literal("mode")
                            .then(literal("static")
                                    .executes(ctx -> {
                                        CONFIG_COMPONENT.partialKeepinvMode(partialKeepinventory.KeepinvMode.STATIC);
                                        modeMessage(ctx);
                                        return 1;
                                    })
                            )
                            ...
                            .then(literal("vanilla")
                                    .executes(ctx -> {
                                        CONFIG_COMPONENT.partialKeepinvMode(partialKeepinventory.KeepinvMode.VANILLA);
                                        modeMessage(ctx);
                                        return 1;
                                    })
                            )
                    ...
                    .then(literal("droprate")
                            .then(literal("static")
                                    ...
                                    .executes(ctx -> {
                                        percentMessage(ctx, "Inventory droprate", CONFIG_COMPONENT.inventoryDroprate());
                                        return 1;
                                    })
                            )
                            .then(literal("common")
    ...
    //                )
                ...
                .then(literal("xp")
                        .then(literal("mode")
                                .then(literal("vanilla")
                                        .executes(ctx -> {
                                            CONFIG_COMPONENT.setKeepxpMode(partialKeepinventory.KeepXPMode.VANILLA);
                                            return 1;
                                        })
                                )
                                .then(literal("static-level")
                                        .executes(ctx -> {
                                            CONFIG_COMPONENT.setKeepxpMode(partialKeepinventory.KeepXPMode.STATIC_LEVEL);
                                ...
                                .then(literal("static-points")
                                        .executes(ctx -> {
                                            CONFIG_COMPONENT.setKeepxpMode(partialKeepinventory.KeepXPMode.STATIC_POINTS);
                                            ctx.getSource().sendMessage(Text.literal("XP mode is set to " + CONFIG_COMPONENT.getKeepxpMode().toString()));
                                            return 1;
                                        })
                                )
